document.addEventListener("DOMContentLoaded", () => {
    ...
    const routes = {
        ...
        deleteItem: (id) =>
            window.PegawaiApp?.routes?.deleteItemBase
                ? `${window.PegawaiApp.routes.deleteItemBase}/${id}`
    ...
    scanForm.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            if (res.ok && json.success) {
                ...
                if (card) {
                    const stokEl = card.querySelector(
                ...
                showToast(
                    "success",
                    json.message || "Barang ditambahkan ke keranjang!"
                );
            } else {
                showAlert(
                    "error",
                    "Gagal",
                    json.message || "Barcode tidak cocok!"
                );
            }
        } catch (err) {
            console.error(err);
        ...
        saveBtn.addEventListener("click", async (e) => {
            ...
            try {
                ...
                if (res.ok && json.success) {
                    showToast(
                        "success",
                        json.message || "Data disimpan ke Item Out!"
                    );
    ...
    async function loadCart() {
        ...
        try {
            ...
            const weeklyCount = json.data?.weekly_request_count ?? 0;
            const reachedLimit = json.data?.has_reached_limit === true;
            const limitMessage = json.data?.limit_message ??
                `Pegawai ini telah mencapai batas maksimal pengajuan mingguan (${weeklyCount}/5)!`;
            ...
            if (reachedLimit) {
                ...
                saveBtn.innerHTML =
                    '<i class="ri-error-warning-line me-1"></i> Tidak Bisa Simpan (Batas Mingguan Tercapai)';
                updateCartBadge();
                return;
            }
                ...
                btn.addEventListener("click", async () => {
                    ...
                    try {
                        const res = await fetch(
                            routes.deleteItem(btn.dataset.id),
                            {
                                method: "DELETE",
                                headers: {
                                    "X-CSRF-TOKEN": csrf,
                        ...
                        if (r.success) {
                            ...
                            showAlert(
                                "error",
                                "Gagal",
                                r.message || "Tidak dapat menghapus item."
                            );
                        }
                    } catch (err) {
                        console.error(err);
